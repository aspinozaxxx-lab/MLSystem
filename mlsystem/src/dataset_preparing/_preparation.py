...
from ._identity import compute_dataset_identity, dataset_identity_mlflow_payload
from .contracts import TrainingDatasetPrepareRequest, TrainingDatasetPrepareResult


def prepare_training_dataset(request: TrainingDatasetPrepareRequest) -> TrainingDatasetPrepareResult:
    output_dir = request.output_dir
    output_dir.mkdir(parents=True, exist_ok=True)
    inventory_path = output_dir / "inventory_scenes.json"
    matching_path = output_dir / "scene_matching_report.json"
    matched_path = output_dir / "matched_scenes.txt"
    manifest_path = output_dir / "dataset_manifest.json"
    audit_json_path = output_dir / "prepare_dataset_input_audit.json"
    audit_txt_path = output_dir / "prepare_dataset_input_audit.txt"
    excluded_path = output_dir / "excluded_dataset_scenes.txt"

    inventory = request.inventory or read_json(inventory_path, default=None) or {}
    matching = request.matching or read_json(matching_path, default=None) or {}
    ...
    limit_info = _resolve_dataset_input_limit(request)
    ...
    if input_errors:
        return TrainingDatasetPrepareResult(
            "failed",
            checks=[
                _check(
                    "input lineage",
    ...
    if not annotation_uri:
        return TrainingDatasetPrepareResult("failed", errors=["annotation_uri is missing in inventory"])

    annotation_path = output_dir / "dataset_annotation.geojson"
    try:
        annotation_path.write_text(read_s3_text(pipeline_config, annotation_uri), encoding="utf-8")
    except Exception as exc:  # noqa: BLE001
        return TrainingDatasetPrepareResult("failed", errors=[f"annotation_file cannot be read: {exc}"])
    ...
    preprocess = request.preprocess or {}
    ...
    try:
        ...
        return TrainingDatasetPrepareResult("failed", errors=[f"Object counting failed: {exc}"])

    split_strategy = _resolve_split_strategy(request, preprocess)
    ...
    if split_strategy == "legacy_75_25":
        ...
        if train_names & val_names:
            return _failed_result("train/val split has overlapping scenes")
    else:
        return _failed_result(f"Unsupported preprocess.split_strategy: {split_strategy}")
    ...
    manifest = {
        ...
        "annotations": inventory.get("annotations") or request.annotations or {},
    ...
    scene_counts_path = output_dir / "scene_object_counts.txt"
    split_report_path = output_dir / "train_val_split.txt"
    train_path = output_dir / "train_scenes.txt"
    val_path = output_dir / "val_scenes.txt"
    summary_path = output_dir / "split_summary.json"
    validation_path = output_dir / "dataset_validation_report.json"
    ...
    write_split_outputs(rows, output_split, output_dir)
    ...
    dataset_identity = compute_dataset_identity(
        ...
        inventory=inventory,
        scene_matching=matching,
        class_name=request.class_name or request.annotations.get("class_name") or request.annotations.get("class"),
        class_slug=request.class_slug or request.annotations.get("class_slug"),
        images_uri=inventory.get("images_uri") or matching.get("images_uri"),
        layout_uri=inventory.get("layout_uri") or matching.get("layout_uri"),
        scenes_uri=inventory.get("scenes_uri") or matching.get("scenes_uri"),
        annotation_uri=annotation_uri,
    )
    mlflow_params = dataset_identity_mlflow_payload(dataset_identity)
    ...
    checks = [
        ...
        _check("inventory", "ok", f"{len(matched)} matched scenes loaded"),
        _check("object counts", "ok", f"{sum(row.object_count for row in rows)} objects counted"),
        _check("split", "failed" if errors else "ok", f"train={len(train_rows)}, val={len(val_rows)}"),
    ]
    return TrainingDatasetPrepareResult(
...
def _resolve_dataset_input_limit(request: TrainingDatasetPrepareRequest) -> dict[str, Any]:
    preprocess = dict(request.preprocess or {})
    raw_preprocess = dict(request.raw_preprocess or {})
    candidates = [
...
def _build_input_audit(
    request: TrainingDatasetPrepareRequest,
    *,
...
def _resolve_split_strategy(request: TrainingDatasetPrepareRequest, preprocess: dict[str, Any]) -> str:
    ...
    if request.schema_version is not None and int(request.schema_version) >= 2:
        return "object_balanced"
    return "legacy_75_25"
...
def _failed_result(error: str) -> TrainingDatasetPrepareResult:
    return TrainingDatasetPrepareResult("failed", errors=[error])
