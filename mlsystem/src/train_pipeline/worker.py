...
from .run_store import TrainPipelineRunStore, utc_now


def run_worker(run_id: str, run_root: str | Path | None = None) -> int:
    faulthandler.enable(all_threads=True)
    store = TrainPipelineRunStore(run_root, run_id)
...
def _mark_stage_running(store: TrainPipelineRunStore, run_id: str, config: PipelineRunConfig, stage: str, index: int, completed: set[str]) -> None:
...
def _mark_failed(store: TrainPipelineRunStore, run_id: str, stage: str, exc: Exception) -> None:
...
def _finish_failed_status(store: TrainPipelineRunStore, run_id: str, message: str, tb: str | None = None) -> None:
...
def _mark_cancelled(store: TrainPipelineRunStore, run_id: str, completed: set[str], config: PipelineRunConfig) -> None:
    finished_at = utc_now()
...
def _read_stage_report(store: TrainPipelineRunStore, stage: str) -> dict[str, Any] | None:
...
def _write_final_summary(store: TrainPipelineRunStore, run_id: str, final: dict[str, Any], config: PipelineRunConfig) -> None:
    summary = store.read_summary()
    summary.update(
        {
            "run_id": run_id,
