...
class TrainPipelineRunner:
    def __init__(self, store: TrainPipelineRunStore | None = None) -> None:
        self.store = store or TrainPipelineRunStore()
    ...
    def refresh_run(self, run_id: str) -> PipelineRun:
        ...
        if payload.get("state") in {"queued", "running"} and payload.get("pid") and not _pid_running(int(payload["pid"])):
            ...
            if latest.get("state") in {"queued", "running"}:
                ...
                self.store.append_log(run_id, f"\n[train_pipeline] {message} diagnostic={diagnostic}\n")
...
def _worker_exit_diagnostic(store: TrainPipelineRunStore, run_id: str, payload: dict[str, Any]) -> dict[str, Any]:
