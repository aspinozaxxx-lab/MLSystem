...
from ..pipeline.contracts import PredictionTile, ProbabilityMap, TileWindow
from .windows import tile_insert_slices, tile_weight_window
...
class ProbabilityStitcher:
    def __init__(self, scene_id: str, config: StitchingConfig) -> None:
    ...
    def result(self, transform: object | None = None, crs: str | None = None) -> ProbabilityMap:
...
def stitch_prediction_tiles(scene_id: str, scene_width: int, scene_height: int, patch_size: int, tiles: list[PredictionTile], **kwargs: object) -> ProbabilityMap:
    stitcher = ProbabilityStitcher(scene_id, StitchingConfig(scene_width=scene_width, scene_height=scene_height, patch_size=patch_size, **kwargs))
    for tile in tiles:
        stitcher.add_prediction_tile(tile)
