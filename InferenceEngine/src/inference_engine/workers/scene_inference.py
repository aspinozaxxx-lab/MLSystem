...
class SceneInferenceRunner:
    def __init__(self, config: Any, model: torch.nn.Module, device: torch.device, inference_config: SceneInferenceConfig) -> None:
        self.config = config
        self.model = model
        self.device = device
    ...
    def run_scene(self, scene_index: int, match: Any) -> SceneInferenceResult:
        ...
        aws = _aws_session_if_available(self.config)

        try:
            env_kwargs = {"AWS_HTTPS": "NO", "AWS_VIRTUAL_HOSTING": "FALSE", "GDAL_CACHEMAX": 64}
            env = rasterio.Env(aws, **env_kwargs) if aws is not None else rasterio.Env(**env_kwargs)
            with env:
                path = _raster_path_for_match(self.config, match)
...
def probability_nonzero_bbox(prob: np.ndarray) -> list[int] | None:
    rows, cols = np.nonzero(prob > 1e-6)
    if rows.size == 0 or cols.size == 0:
        return None
    return [int(cols.min()), int(rows.min()), int(cols.max()) + 1, int(rows.max()) + 1]


def _match_value(match: Any, name: str, default: Any = None) -> Any:
...
def _raster_path_for_match(config: Any, match: Any) -> str:
    path = _match_value(match, "path") or _match_value(match, "local_path")
    if path:
        return str(path)
...
def _aws_session_if_available(config: Any) -> Any | None:
    try:
        from mlsystem.src.storage.s3 import aws_session  # type: ignore

        return aws_session(config)
    except Exception:
        return None
