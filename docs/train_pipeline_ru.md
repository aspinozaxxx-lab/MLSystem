...
```bash
python -m mlsystem.src.train_pipeline.cli run --trace configs/pipeline.smoke.local.yaml
python -m mlsystem.src.train_pipeline.cli status --run-id smoke_pipeline_local
python -m mlsystem.src.train_pipeline.cli log --run-id smoke_pipeline_local
```
