...
from mlsystem.src.train_pipeline.api import PipelineRunConfig, TrainPipelineRunStore
from mlsystem.src.train_pipeline.runner import _pid_running, _worker_exit_diagnostic


class TrainPipelineRunnerStatusTests(unittest.TestCase):
    def test_pid_running_treats_linux_zombie_as_exited(self) -> None:
        with (
            patch("mlsystem.src.train_pipeline.runner._proc_state_for_pid", return_value="Z"),
            patch("mlsystem.src.train_pipeline.runner.os.kill") as kill,
        ):
    ...
    def test_worker_exit_diagnostic_includes_log_tails(self) -> None:
        with TemporaryDirectory() as tmp:
            store = TrainPipelineRunStore(tmp)
