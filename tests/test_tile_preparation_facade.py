...
from mlsystem.src.tile_preparation import AnnotationInput, SceneInput, TilePreparationFacade, TrainingTileDataset
...
class TilePreparationFacadeTests(unittest.TestCase):
    def test_default_config_accepts_all_augmentation_levels(self) -> None:
        for level in range(4):
            with self.subTest(level=level):
                config = TilePreparationFacade.default_config(tile_size=128, stride=128, augmentation_level=level)
    ...
    def test_build_datasets_uses_ready_train_and_val_scenes(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            bundle = TilePreparationFacade.build_datasets(
                train_scenes=[SceneInput(root / "scene_a.tif", "scene_a")],
                val_scenes=[SceneInput(root / "scene_b.tif", "scene_b")],
            ...
            try:
                ...
                self.assertEqual([scene.resolved_scene_id() for scene in bundle.train_dataset.scenes], ["scene_a"])
                self.assertEqual([scene.resolved_scene_id() for scene in bundle.val_dataset.scenes], ["scene_b"])
            finally:
    ...
    def test_missing_geojson_crs_fails_when_inference_is_disabled_in_low_level_dataset(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            ...
            config = TilePreparationFacade.default_config(tile_size=128, stride=128, augmentation_level=0)
            with self.assertRaisesRegex(ValueError, "annotation CRS is missing"):
                dataset = TrainingTileDataset(
                    [SceneInput(root / "scene.tif", "scene")],
                    AnnotationInput(root / "ann.geojson", annotation_crs="auto", allow_inferred_annotation_crs=False),
                    config,
                    train=True,
                )
                dataset.close()
