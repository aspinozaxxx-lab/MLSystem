...
class TrainingReportMLflowReaderTests(unittest.TestCase):
    ...
    def test_current_inventory_dataset_can_match_by_counts_when_fingerprints_differ(self) -> None:
        collector = object.__new__(TrainingReportCollector)
        rows = collector._build_class_rows(  # pylint: disable=protected-access
            {"lakes": {"objects_count": 410, "scenes_count": 28, "dataset_fingerprint": "inventory-fp"}},
            [
                {
                    "run_id": "old_high",
                    "class_slug": "lakes",
